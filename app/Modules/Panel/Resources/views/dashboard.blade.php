...
@section('content')
    <div id="content" class="content">
        ...
        <!-- end breadcrumb -->
        <!-- begin page-header -->
        <h1 class="page-header">Dashboard <small>Panel dashboard & statistik</small></h1>
        <!-- end page-header -->
        <!-- begin row -->
        <div class="row">
            <!-- begin col-3 -->
                ...
                <div class="widget-chart with-sidebar bg-black">
                    <div class="widget-chart-content">
                        <h4 class="chart-title">
                            Analisis Pengunjung
                            <small>Statistik analisis pengunjung selama 14 hari</small>
                        </h4>
                        <div id="visitors-line-chart" class="morris-inverse" style="height: 260px;"></div>
                    </div>
                    <div class="widget-chart-sidebar bg-black-darker">
                        <div class="chart-number">
                            {{ $totalVisitors = $visitors->rows[0][1] + $visitors->rows[1][1] }}
                            <small>pengunjung</small>
                        </div>
                        <div id="visitors-donut-chart" style="height: 160px"></div>
                        <ul class="chart-legend">
                            <li><i class="fa fa-circle-o fa-fw text-success m-r-5"></i> {{ round(($visitors->rows[0][1]/$totalVisitors) * 100, 2 ) }}% <span>Pengunjung Baru</span></li>
                            <li><i class="fa fa-circle-o fa-fw text-primary m-r-5"></i> {{ round(($visitors->rows[1][1]/$totalVisitors) * 100, 2 ) }}% <span>Pengunjung Lama</span></li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        var handleVisitorsLineChart = function() {
            ...
            Morris.Line({
                ...
                data: [
                    @foreach($analyticsData as $index => $data){x: '{{ $data['date']->format('Y-m-j') }}', y: {{ $data['visitors'] }}, z: {{ $data['pageViews'] }}}@if ($index+1 != count($analyticsData)),@endif
                    @endforeach
                ],
                xkey: 'x',
                ykeys: ['y', 'z'],
        ...
        var handleVisitorsDonutChart = function() {
            ...
            Morris.Donut({
                ...
                data: [
                    {label: "Pengunjung Baru", value: {{ $visitors->rows[0][1] }}},
                    {label: "Pengunjung Lama", value: {{ $visitors->rows[1][1] }}}
                ],
                colors: [green, blue],
