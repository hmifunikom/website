                    ...
                    <div class="panel-toolbar">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                {
                                    $divisiLinks[] = [
                                            'url' => route('panel.user.create', ['divisi' => $div->id_divisi]),
                                            'label' => $div->singkatan,
                                            'linkAttributes' => ['data-pjax' => true],
                                    ];
                                ...
                                ?>
                                <form class="form-inline" method="GET" id="search" action="{{ route('panel.user.create') }}" data-pjax="true" style="display: inline-block;">
                                ...
                                <div class="btn-group">
                                    <a class="btn btn-success {{ menu_active_qs('status', 'pengurus') }}" data-pjax href="{{ route_append_qs('panel.user.create', ['status' => 'pengurus'], 'divisi') }}">Pengurus</a>
                                    <a class="btn btn-primary {{ menu_active_qs('status', 'am') }}" data-pjax href="{{ route_append_qs('panel.user.create', ['status' => 'am'], 'divisi') }}">AM</a>
                                </div>
                            </div>
                            ...
                            <table class="table table-hover table-striped">
                                ...
                                @foreach($anggota as $angg)
                                    <tr>
                                        ...
                                        <td>{{ $angg->tempat }}</td>
                                        <td class="text-right">
                                            {!! Button::primary(Icon::plus() . ' Buat user')->small()->asLinkTo(route('panel.user.createtwo', $angg->getWrappedObject()))->withAttributes(['data-pjax']) !!}
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
